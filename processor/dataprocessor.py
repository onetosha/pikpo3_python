...
class DataProcessor(ABC):
    ...
    def run(self) -> None:
        ...
        for item in mas:
            ...
            mas[count] = float(mas[count])
        self.result.replace(self.result['2010'], mas)
        self.result = self.sort_data_by_col(self.result, '2010', False)
        self.result = self.result.head(10)
    ...
    def read(self):
        try:
            ...
            for separator in self.separators:
                self._dataset = pandas.read_csv(self._datasource, on_bad_lines='skip', sep=separator, header='infer', names=None,
                                                encoding="utf-8")
